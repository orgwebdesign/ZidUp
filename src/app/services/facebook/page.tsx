...
import { Users, Heart, Eye, MessageCircle, Zap, ThumbsUp } from "lucide-react";
import type { ElementType } from "react";
import fs from "fs";
import path from "path";

export const metadata: Metadata = {
  title: "Facebook Services — ZID UP",
  description: "Buy Facebook page likes, followers, post likes, and comments. Real engagement at competitive prices.",
};
...
const serviceIcons: Record<string, { icon: ElementType; color: string }> = {
  Followers: { icon: Users, color: "text-blue-600" },
  Views: { icon: Eye, color: "text-blue-600" },
  Comments: { icon: MessageCircle, color: "text-blue-600" },
  PostLikes: { icon: ThumbsUp, color: "text-blue-600" },
  PageLikes: { icon: Heart, color: "text-blue-600" },
};

export default function FacebookServicesPage() {
  const pricing = getPricing();
  const services = pricing?.facebook ? Object.entries(pricing.facebook) : [];
      ...
      <main className="min-h-screen pt-32 pb-20 bg-background">
        <div className="container-custom">
          <div className="text-center mb-16">
            <h1 className="text-h1 font-heading font-extrabold text-white mb-6">Facebook Services</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
            {services.map(([key, service]: [string, PricingService]) => {
              const meta = serviceIcons[key] || { icon: Users, color: "text-blue-600" };
