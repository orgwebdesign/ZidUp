...
import { Music2, Heart, Eye, MessageCircle, Zap } from "lucide-react";
...
const serviceIcons: Record<string, { icon: ElementType; color: string }> = {
  Followers: { icon: Music2, color: "text-white" },
  Likes: { icon: Heart, color: "text-white" },
  Views: { icon: Eye, color: "text-white" },
  Comments: { icon: MessageCircle, color: "text-white" },
};

export default function TikTokServicesPage() {
  const pricing = getPricing();
  const services = pricing?.tiktok ? Object.entries(pricing.tiktok) : [];
        ...
        <div className="container-custom">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
            {services.map(([key, service]: [string, PricingService]) => {
              const meta = serviceIcons[key] || { icon: Music2, color: "text-white" };
